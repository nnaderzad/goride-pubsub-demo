...
output "analytics_sub_console_url" {
  description = "Open analytics-sub (Message transforms section shows the maskEmail SMT + Test transform button)."
  value       = "https://console.cloud.google.com/cloudpubsub/subscription/detail/${google_pubsub_subscription.analytics.name}?project=${local.project_id}"
}
...
output "publish_command" {
  ...
  value       = "gcloud pubsub topics publish ${google_pubsub_topic.rides.name} --project=${local.project_id} --message='{\"event_id\":\"evt_maya\",\"user_id\":\"u_456\",\"rider_email\":\"maya@example.com\",\"driver_id\":\"d_789\",\"event_type\":\"trip_completed\",\"fare\":24.50,\"city\":\"San Francisco\",\"timestamp\":\"2026-07-22T22:15:00Z\"}'"
}

output "run_this_query" {
  description = "Paste into the BigQuery console to show events that landed (rider_email arrives masked - that's the SMT)."
  value       = "SELECT event_type, rider_email, city, fare, timestamp FROM `${local.project_id}.${google_bigquery_dataset.analytics.dataset_id}.${google_bigquery_table.trip_events.table_id}` ORDER BY timestamp DESC LIMIT 20"
}
